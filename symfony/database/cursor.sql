drop procedure if exists LogArchive;
delimiter //
create procedure LogArchive()
begin
	declare finished integer default 0;
	declare f_id integer;
	declare f_user integer;
	declare f_user_uuid varchar(128);
	...
	declare log_cursor cursor for
		select l.id, l.user, l.userUUID, l.type, l.message, l.timestamp
		from Log as l
	...
	move_log: loop
		fetch log_cursor into f_id, f_user, f_user_uuid, f_type, f_message, f_timestamp;
		if finished then
			leave move_log;
		end if;
		insert into LogArchive set
			user = f_user,
			userUUID = f_user_uuid,
			type = f_type,
			message = f_message,
